...
NOTE: Lorsque vous choisissez d'envoyer le message AutoSupport par BlueXP , il peut prendre jusqu'à cinq heures car le chargement de BlueXP  s'équilibre automatiquement lorsque les messages AutoSupport sont envoyés. Si votre communication avec le support NetApp est rapide, téléchargez le fichier et envoyez-le vous-même au support NetApp.
