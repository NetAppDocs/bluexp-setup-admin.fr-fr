...
NOTE: Si vous souhaitez utiliser BlueXP dans le https://["L'environnement AWS commercial Cloud Services (C2S)"^] Vous devez ensuite suivre des instructions séparées qui décrivent la préparation, l'installation du connecteur et le lancement de Cloud Volumes ONTAP. https://["Découvrez comment vous lancer avec Cloud Volumes ONTAP dans l'environnement AWS C2S"^]
...
| Pour gérer les ressources dans AWS. Le point final exact dépend de la région dans laquelle vous déployez le connecteur. https://["Pour plus d'informations, consultez la documentation AWS"^] 
...
https://["Documentation Azure : référence IP publique"^]
...
+
https://["Documentation Microsoft Azure : configurez les identités gérées des ressources Azure sur une machine virtuelle à l'aide du portail Azure"^]
...
.Étape
. https://["Activez les API Google Cloud suivantes dans votre projet"^]
+
