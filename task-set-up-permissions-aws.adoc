...
Si nécessaire, vous pouvez restreindre la politique IAM à l'aide de l'IAM `Condition` elément. https://["Documentation AWS : élément de condition"^]
...
+
Pour rappel, cette règle contient uniquement les autorisations nécessaires pour lancer l'instance Connector dans AWS à partir de BlueXP. link:reference-permissions-aws.html["Autorisations de vue requises pour l'instance de connecteur"].
