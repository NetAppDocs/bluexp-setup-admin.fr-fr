...
[role="lead"]
Un compte _BlueXP_ fournit une colocation pour votre entreprise, qui vous permet d'organiser les utilisateurs et les ressources dans des _espaces de travail_ isolés. Par exemple, un groupe d'utilisateurs peut déployer et gérer des environnements de travail Cloud Volumes ONTAP dans un espace de travail qui n'est pas visible pour les utilisateurs qui gèrent des environnements de travail dans un autre espace de travail.
