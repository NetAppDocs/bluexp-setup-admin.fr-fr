...
+
L'ajout d'identifiants AWS à un connecteur existant offre les autorisations nécessaires pour gérer les ressources et les processus au sein de votre environnement de cloud public. <<Add credentials to a Connector,Découvrez comment ajouter des identifiants AWS à un connecteur>>.
...
+
L'ajout de nouvelles informations d'identification AWS à BlueXP offre à BlueXP les autorisations nécessaires pour créer et gérer FSX pour ONTAP. https://["Découvrez comment configurer des autorisations pour FSX pour ONTAP"^]
...
+
*** https://["Affichez les autorisations nécessaires pour FSX pour ONTAP"^]
*** link:task-set-up-permissions-aws.html["Afficher la règle de déploiement des connecteurs"]
...
Pour plus de détails, reportez-vous à la https://["Documentation BlueXP pour Amazon FSX pour ONTAP"^]
