...
[NOTE]
====
* Des notifications sont alors envoyées par e-mail pour les fonctionnalités et services BlueXP suivants : connecteurs, Cloud Sync, Cloud Backup, portefeuille digital et protection contre les ransomwares. D'autres services seront ajoutés dans les prochaines versions.
...
Pour ignorer toutes les notifications, dans le Centre de notification, sélectionnez image:button_3_vert_dots.png["une icône avec trois points verticaux"] Et sélectionnez *rejeter tout*.image:screenshot_notification_menu.png["Capture d'écran affichant le menu Notifications pour sélectionner les paramètres et ignorer toutes les options."]

Pour ignorer les notifications individuelles, placez le curseur sur la notification et sélectionnez *rejeter*.image:screenshot_notification_dismiss1.png["Capture d'écran montrant comment rejeter une seule notification."]
