...
[NOTE]
====
* Les notifications sont alors envoyées par e-mail pour les fonctionnalités et services BlueXP suivants : connecteurs, Cloud Sync, sauvegarde dans le cloud et protection contre les attaques par ransomware. D'autres services seront ajoutés dans les prochaines versions.
...
Pour ignorer toutes les notifications, dans le Centre de notification, cliquez sur image:button_3_vert_dots.png["une icône avec trois points verticaux"] Et sélectionnez *rejeter tout*.image:screenshot_notification_menu.png["Capture d'écran affichant le menu Notifications pour sélectionner les paramètres et ignorer toutes les options."]
