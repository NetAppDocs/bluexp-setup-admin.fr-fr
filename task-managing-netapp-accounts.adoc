...
https://["Affichez les terminaux de l'API de colocation"^]
...
.Étapes
. Si l'utilisateur ne l'a pas déjà fait, demandez-lui d'aller à https://["Site Web NetApp BlueXP"^] et s'inscrire.
...
Pour passer des appels API à https://["API de location"^], vous devrez obtenir un jeton de porteur pour un compte de service.

https://["Découvrez comment créer un jeton de compte de service"^]
