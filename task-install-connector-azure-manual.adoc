...
[role="lead"]
Vous pouvez installer manuellement un connecteur sur un hôte Linux s'exécutant dans Azure. Pour installer manuellement le connecteur sur votre propre hôte Linux, vous devez vérifier la configuration requise pour l'hôte, configurer votre réseau, préparer les autorisations Azure, installer le connecteur, puis fournir les autorisations que vous avez préparées.
