...
[role="lead"]
Fournissez les informations d'identification nécessaires à vos comptes sur le site de support NetApp (NSS) pour s'inscrire au support, activer les workflows clés pour Cloud Volumes ONTAP et bien plus encore. Ces informations d'identification NSS sont associées à l'ensemble du compte BlueXP.
...
* https://["Lancement d'Cloud Volumes ONTAP dans AWS"^]
* https://["Lancement d'Cloud Volumes ONTAP dans Azure"^]
* https://["Lancement d'Cloud Volumes ONTAP dans Google Cloud"^]
* https://["Enregistrement des systèmes de paiement à l'utilisation"^]
...
.Étapes
. Si ce n'est déjà fait, https://["Créez un compte Microsoft Azure Active Directory B2C qui sera lié à votre compte BlueXP actuel"^].
. Dans le coin supérieur droit de la console BlueXP, sélectionnez l'icône aide, puis sélectionnez *support*.
