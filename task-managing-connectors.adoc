...
AWS:: Lorsque vous avez créé l'instance Connector dans AWS, vous avez fourni une clé d'accès AWS et une clé secrète. Vous pouvez utiliser cette paire de clés vers SSH à l'instance.
+
--
https://["AWS Docs : connectez-vous à votre instance Linux"^]

--
Azure:: Lorsque vous avez créé la machine virtuelle Connector dans Azure, vous avez choisi de vous authentifier avec un mot de passe ou une clé publique SSH. Utiliser la méthode d'authentification que vous avez choisie pour vous connecter à la machine virtuelle.
+
--
https://["Azure Docs : connexion SSH à votre machine virtuelle"^]
...
+
--
https://["Google Cloud Docs : connectez-vous aux machines virtuelles Linux"^]
...
NOTE: Sur les hôtes disposant d'un accès Internet, le connecteur met automatiquement à jour son logiciel vers la dernière version, tant qu'il dispose d'un accès Internet sortant pour obtenir la mise à jour du logiciel.
...
https://["En savoir plus sur l'appel API /ocm/config"^]
