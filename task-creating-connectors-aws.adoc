...
[role="quick-margin-para"]
Vous avez besoin d'un VPC et d'un sous-réseau avec un accès Internet sortant à des terminaux spécifiques. Si un serveur proxy est requis pour l'Internet sortant, vous aurez besoin de l'adresse IP, des informations d'identification et du certificat HTTPS.
...
** *Network* : spécifiez un VPC, un sous-réseau et une paire de clés pour l'instance, choisissez d'activer ou non une adresse IP publique, et spécifiez éventuellement une configuration proxy (HTTP et HTTPS sont pris en charge).
+
...
Si vous disposez de compartiments Amazon S3 sur le même compte AWS que celui sur lequel vous avez créé le connecteur, l'environnement de travail Amazon S3 s'affiche automatiquement sur la fenêtre Canvas. https://docs.netapp.com/us-en/bluexp-s3-storage/index.html["Découvrez ce que vous pouvez faire dans cet environnement de travail"].
